...
async function executeScalpSignal(signal, optPriceEst = 0, volumePct = 1.0, underlyingPrice = 0, etfCtx = null) {
  ...
  // Tracker (_qqqLastFire / DIRECTION_CONFLICT_MS) preserved at module scope.
  ...
  if (!fill.vetoed) {
    ...
    _qqqLastFire.ms = Date.now(); _qqqLastFire.dir = dir;
...
async function poll() {
  ...
  if (!QQQ_SUSPENDED) {
    ...
    // above still runs for jSignal audit + printSummary + qqq-levels.json.
