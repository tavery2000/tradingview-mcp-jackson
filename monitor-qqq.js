#!/usr/bin/env node
/**
 * monitor-qqq.js — QQQ + W3 continuous market monitor
 * Built by NYC2000
 *
 ...
 *   - Fires CALLS/PUTS when 4+/5 W3 stocks agree + QQQ confirms
...
const activeSwing = { QQQ: { requestId: null, status: null } };
...
const INSTRUMENT   = 'QQQ';
const COMPONENTS   = ['AMD', 'AVGO', 'TSLA', 'ARM', 'NVDA'];  // W3 stocks (excludes QQQ itself)
const ALL_STOCKS   = ['QQQ', ...COMPONENTS];
const CHART_ID     = cfg.qqqChartId ?? 'f4zZcgs4';
const LEVELS_FILE  = join(__dirname, 'qqq-levels.json');

const POLL_MS      = 30_000;
const THRESHOLD    = 3;          // 3/5 components + QQQ confirms
...
async function initClient() {
  ...
  console.log(`  Found ${charts.length} TradingView tab(s) — looking for QQQ tab (id=${CHART_ID})...`);
  ...
  if (!target) {
    ...
    for (const t of charts) {
      try {
        ...
        if (/qqq/i.test(sym.result?.value ?? '')) { target = t; await c.close(); break; }
        await c.close();
      } catch {}
    }
  }

  if (!target) throw new Error(`QQQ tab not found. Open the Claude QQQ tab in TradingView.`);

  client = await CDP({ host: 'localhost', port: 9222, target: target.id });
  await client.Runtime.enable();
  console.log(`  QQQ tab connected: ${target.url.split('/chart/')[1]?.replace('/', '')}`);
}
...
function printPaperPanel(divLine) {
  ...
  try {
    ...
    _lastGoodLedgerQQQ = ledger;
  } catch {
    ledger = _lastGoodLedgerQQQ;
    if (!ledger) {
      console.log(divLine);
...
const SwingEngine = (() => {
  ...
  function update(price, vwap, ema9, bars) {
    ...
    if(state.status==='WAITING'&&!state.openedToday&&isOpening()&&!consumed&&bars.length>0&&vwap!=null&&ema9!=null) {
      ...
      else{consumed=false;console.log(`  ${C.gray}[QQQ SWING] No clear bias at open — watching...${C.reset}`);}
    }
...
function buildSignal(compRows, etf, summary) {
  ...
  if (compBulls >= THRESHOLD && bullish && etfVrrsBull && (approachingSup||breakout))
    return { action:'TAKE CALLS 🟢', confidence:'HIGH', reason:`${compBulls}/${total} BULL + QQQ bullish${vrrsNote} + ${breakout?'breakout':'near support'}`, bulls:compBulls, bears:compBears, total };
  if (compBears >= THRESHOLD && bearish && etfVrrsBear && (approachingRes||breakdown))
    return { action:'TAKE PUTS 🔴', confidence:'HIGH', reason:`${compBears}/${total} BEAR + QQQ bearish${vrrsNote} + ${breakdown?'breakdown':'near resistance'}`, bulls:compBulls, bears:compBears, total };
  if (compBulls >= THRESHOLD && bullish) return { action:'TAKE CALLS 🟢', confidence:'MEDIUM', reason:`${compBulls}/${total} BULL + QQQ bullish${vrrsNote}`, bulls:compBulls, bears:compBears, total };
  if (compBears >= THRESHOLD && bearish) return { action:'TAKE PUTS 🔴', confidence:'MEDIUM', reason:`${compBears}/${total} BEAR + QQQ bearish${vrrsNote}`, bulls:compBulls, bears:compBears, total };
  if (compBulls >= THRESHOLD) return { action:'CALLS — WAIT ⚠️', confidence:'WEAK', reason:`${compBulls}/${total} BULL but QQQ not confirming (${etf?.bias})`, bulls:compBulls, bears:compBears, total };
  if (compBears >= THRESHOLD) return { action:'PUTS — WAIT ⚠️', confidence:'WEAK', reason:`${compBears}/${total} BEAR but QQQ not confirming (${etf?.bias})`, bulls:compBulls, bears:compBears, total };
...
async function fetchAnalysis(etf, compRows, swingState, summary) {
  ...
  const cached = analysisCache.get('QQQ');
  ...
  const prompt = `You are HANK, an AI trading assistant. Provide a concise 2-3 sentence market analysis for QQQ right now. Be direct and actionable. No preamble.

QQQ price: $${etf.price?.toFixed(2)} | VWAP: $${etf.vwap?.toFixed(2)} | Delta: ${etf.delta!=null?(etf.delta/1000).toFixed(1)+'K':'N/A'} | Bias: ${etf.bias}
TICK: ${etf.tick ?? 'N/A'}
W3 components: ${comps}
...
Write as if speaking to a trader watching the screen. Focus on what matters right now.`;
  ...
  try {
    ...
    if (text) analysisCache.set('QQQ', { text, hash, ts: Date.now() });
...
let _lastVolumePct = 1.0; // module-level so printSummary can access it
...
function printSummary(compRows, etf, summary, swingState, signal, analysis, structureSig) {
  ...
  console.log(C.bold + `  ║  QQQ MONITOR  │  ${getETString()} ET  │  BULL ${pureBulls}/5  BEAR ${pureBears}/5        ║` + C.reset);
  console.log(C.bold + `  ╠══════════════════════════════════════════════════════════════════╣` + C.reset);
  ...
  // Swing Engine
  console.log(line);
  console.log(`  ${C.bold}QQQ SWING ENGINE${C.reset}  ${C.dim}ATR-based · 2.5:1 R:R${C.reset}`);
  const sw = swingState;
  ...
  console.log(C.bold + `  ╚══════════════════════════════════════════════════════════════════╝` + C.reset);
  console.log(`  ${C.dim}Threshold: ${THRESHOLD}/5 + QQQ  │  VRRS: ±${VRRS_THRESH}  │  Poll: ${POLL_MS/1000}s  │  Ctrl+C to quit${C.reset}\n`);

  // HANK LIVE ANALYSIS
  if (analysis) {
    console.log(`  ${C.bold}${C.cyan}◉ HANK LIVE ANALYSIS${C.reset}  ${C.dim}QQQ · 30s · ${getETString()} ET${C.reset}`);
...
const QQQ_SUSPENDED  = true;     // monitor.js owns QQQ trading — this process is data-only
const lastScalpOrder = { QQQ: 0 };
const SCALP_COOLDOWN = 300_000;  // 5 minutes between scalp orders

async function executeSwingEntry(swingState) {
  if (!sendOrder || !orderGate || activeSwing.QQQ.requestId) return;
  ...
  try {
    if (selectContract) {
      const c   = selectContract('QQQ', price, signal);
      strike    = c.strike;
      expiry    = c.expiry;
  ...
  // Stack confidence with QQQ's 4H macro + today's bias regime.
  let macro4H = 'UNKNOWN';
  if (barCache) {
    try {
...
function checkTrendExits(currentEst) {
  ...
  const openTrend = (lg.trades ?? []).filter(t => t.instrument === 'QQQ' && t.engine === 'TREND' && t.status === 'OPEN');
  ...
  for (const t of openTrend) {
    ...
    } else if (currentEst > 0 && currentEst <= fill * 0.5) {
      closePosition(t.requestId, parseFloat((fill * 0.5).toFixed(4)), 'STOP_0.5X');
...
async function executeScalpSignal(signal, optPriceEst = 0, volumePct = 1.0, underlyingPrice = 0, etfCtx = null) {
  ...
  if (!sendOrder || !orderGate) { jGateBlock(sigEngine, 'QQQ', sigDir, 'TRADE_DISABLED', { macro4H }); return; }
  if (!isTradingHours())        { jGateBlock(sigEngine, 'QQQ', sigDir, 'OUT_OF_HOURS',   { macro4H }); return; }
  ...
  try {
    ...
    const openCount = (lg.trades ?? []).filter(t => t.instrument === 'QQQ' && t.status === 'OPEN').length;
    if (openCount >= 3) { jGateBlock(sigEngine, 'QQQ', sigDir, 'INSTRUMENT_CAP', { open: openCount, macro4H }); return; }
  } catch {}
  const dir = signal.action.includes('CALLS') ? 'CALLS' : signal.action.includes('PUTS') ? 'PUTS' : null;
  if (!dir) { jGateBlock(sigEngine, 'QQQ', sigDir, 'NO_DIRECTION', { action: signal.action, macro4H }); return; }
  ...
  if (gate.block) {
    jGateBlock(sigEngine, 'QQQ', dir, gate.reason, {
  ...
  if (macro4HGate.block) {
    jGateBlock(sigEngine, 'QQQ', dir, macro4HGate.reason, { macro4H, signalEngine: sigEngine });
    return;
  }
  ...
  if (vwapGate.block) {
    jGateBlock(sigEngine, 'QQQ', dir, vwapGate.reason, { price: underlyingPrice, vwap: _vwap, macro4H });
    return;
  }
...
async function poll() {
  ...
  // Per-poll QQQ macro4H — for SIGNAL journal records under HIERARCHY_V2.
  let _qqqMacro4H = 'UNKNOWN';
  if (barCache) {
    try {
      const bars4H = await barCache.get('240');
      if (bars4H && bars4H.length) _qqqMacro4H = analyze4H(bars4H).direction;
    } catch {}
  }
  ...
  try {
    jPoll({
      monitor:  'qqq',
      qqq:      etf ? { price: etf.price ?? null, vwap: etf.vwap ?? null, delta: etf.delta ?? null, bias: etf.bias ?? null, bars: (etf.bars?.length ?? 0) } : null,
      w3:       compRows.map(r => ({ sym: r.symbol, bias: r.bias, delta: r.delta })),
      ...
      suspended: QQQ_SUSPENDED,
      macro4H:  _qqqMacro4H,
    });
    if (signal && (signal.action.includes('CALLS') || signal.action.includes('PUTS')))
      jSignal('TREND', signal.action.includes('CALLS') ? 'CALLS' : 'PUTS', signal.confidence, signal.reason, { instrument: 'QQQ', macro4H: _qqqMacro4H });
    if (structureSig)
      jSignal('STRUCTURE', structureSig.action, structureSig.confidence, structureSig.reason, { instrument: 'QQQ', event: structureSig.event, macro4H: _qqqMacro4H });
  } catch (e) { jError('qqq-poll-journal', e.message); }

  // Swing order wiring — gated by QQQ_SUSPENDED
  if (!QQQ_SUSPENDED) {
    if (swingState.status === 'LONG' || swingState.status === 'SHORT') {
      if (!activeSwing.QQQ.requestId) await executeSwingEntry(swingState);
    }
    if (swingState.status === 'CLOSED' && activeSwing.QQQ.requestId) executeSwingExit(swingState);
  }
  ...
  const _volumePct  = _bars.length > 0 ? _sessionVol / (QQQ_AVG_VOL_PER_BAR * _bars.length) : 1.0;
  _lastVolumePct    = _volumePct;

  // Scalp signal — gated by QQQ_SUSPENDED (monitor.js now owns QQQ trading).
  // HIERARCHY_V2: buildSignal output (TREND consensus) no longer dispatches —
  // it becomes a confidence input. Chart engines (STRUCTURE/FVG/SWEEP) are the
  // only paths that fire orders.
  if (!QQQ_SUSPENDED) {
  ...
  if (signal.confidence === 'HIGH' || signal.confidence === 'MEDIUM') {
    ...
    if (dir && (lastAlert !== dir || now - lastAlertTime > COOLDOWN)) {
      ...
      console.log(`\n  ${dir==='CALLS'?C.bgGreen:C.bgRed}  *** QQQ ${dir} ***  ${C.reset}  ${signal.reason}\n`);
...
async function main() {
  console.log(C.bold + '\n  QQQ Market Monitor — Starting up...' + C.reset);
  console.log(`  Instrument: QQQ`);
  console.log(`  W3 stocks:  ${COMPONENTS.join(', ')}`);
  console.log(`  Chart ID:   ${CHART_ID}`);
  console.log(`  Hours:      7:00 AM – 4:00 PM ET  |  Poll: ${POLL_MS/1000}s`);
  console.log(`  Threshold:  ${THRESHOLD}/5 components + QQQ confirms`);
  ...
  barCache = createBarCache({ evalOn, instrumentLabel: 'QQQ' });
  ...
  try {
    ...
    if (r.ok) {
      console.log(`  ${C.green}✓${C.reset} QQQ: ${summary}`);
    } else {
      console.log(`  ${C.yellow}⚠${C.reset} QQQ: ${summary} — missing: ${r.missing.join(',')}`);
      jAlert('warn', 'QQQ bar bootstrap incomplete', { missing: r.missing, status });
    }
  } catch (e) {
    console.error(`  ${C.red}✗${C.reset} QQQ bar bootstrap failed: ${e.message}`);
    jError('bars-bootstrap-fatal', e.message, { instrument: 'QQQ' });
  }
  console.log('');
  ...
  try {
    ...
    const _sw = _lg.trades?.find(t => t.instrument === 'QQQ' && t.engine === 'SWING' && t.status === 'OPEN');
    if (_sw) { activeSwing.QQQ = { requestId: _sw.requestId, status: 'OPEN' }; console.log(`  [SWING] Re-hydrated open QQQ swing — reqId ${_sw.requestId}`); }
  } catch {}
