...
async function executeSwingEntry(swingState) {
  ...
  if (!fill.vetoed) {
    ...
    activeSwing.IWM = { requestId: reqId, status: 'OPEN', entry: price, strike, expiry, optionEntry: optionMid };
...
function executeSwingExit(swingState) {
  ...
  // BUG FIX 2026-05-12: swingState.exitPrice is the UNDERLYING IWM price at
  // target/stop, NOT an option premium. Passing it directly to closePosition
  // caused phantom $28K profits when underlying (~282) was treated as option
  // exit price vs entry (~0.12). Mirror the SPY fix at monitor.js:2282-2298 —
  // convert underlying move to option-space using a delta approximation.
  const active = activeSwing.IWM;
  ...
  console.log(`  [SWING] IWM closed — ${swingState.exitReason} | underlying $${swingState.exitPrice?.toFixed(2)} → option est $${optionExitPrice}`);
...
async function poll() {
  ...
  try {
    ...
    if (client) await maybeSwitchTimeframe(client, { name: 'monitor-iwm.js' });
  } catch {}
  ...
  let _iwm1H = { trendBias: 'NEUTRAL', structurePattern: 'NEUTRAL' };
  if (barCache) {
    try {
      const bars1H = await barCache.get('60');
      if (bars1H && bars1H.length) {
        const a = analyze1H(bars1H, etf?.price);
        _iwm1H = { trendBias: a.trendBias, structurePattern: a.structurePattern };
      }
    } catch {}
  }
  try {
    writeFileSync(join(__dirname, 'macro1h-iwm.json'), JSON.stringify({
      instrument: 'IWM', ..._iwm1H, ts: Date.now(), time: getETString(),
    }));
  } catch {}
